...
describe('Service: vnApi', function () {
    ...
    describe('should have a resource for', function () {
        ...
        it('Categories', function () {
            ...
            expect(function () {
                vnApi.getCategory();
            }).toThrow(new Error('The Category $resource needs an id.'));
        });
        ...
        it('Nav', function () {
            ...
            expect(function () {
                vnApi.Nav();
            }).toThrow(new Error('The Nav $resource needs a navId'));

        });

        it('Products', function () {
            expect(typeof vnApi.getProduct({code: 'GF-honey'})).toBe('object');
            expect(vnApi.getProduct.name).toBe('getProduct');
            expect(function () {
                vnApi.getProduct();
            }).toThrow(new Error('The Product $resource needs a code.'));
        });
    });
